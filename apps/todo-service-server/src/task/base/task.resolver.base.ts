...
export class TaskResolverBase {
  ...
  async createTask(@graphql.Args() args: CreateTaskArgs): Promise<Task> {
    return await this.service.createTask({
      ...
      data: {
        ...
        taskList: args.data.taskList
          ? {
              connect: args.data.taskList,
            }
          : undefined,
      },
    });
  ...
  @graphql.ResolveField(() => TaskList, {
    nullable: true,
    name: "taskList",
  })
  async getTaskList(@graphql.Parent() parent: Task): Promise<TaskList | null> {
    const result = await this.service.getTaskList(parent.id);
